-- Copyright 2014 ingeniousclown, katkat42 
...
function SLTrait.WillCharacterKnowTrait(craftingSkillType, researchLineIndex, traitIndex)
...
function SLTrait.GetItemTraitResearchabilityInfo(itemLink)
    local itemType = GetItemLinkItemType(itemLink)
    if itemType ~= ITEMTYPE_ARMOR and itemType ~= ITEMTYPE_WEAPON then
        return 0, false, "WrongItemType"
    end
    
    local craftingSkillType, researchLineIndex, traitIndex = SLTrait.GetItemResearchInfo(itemLink)
    ...
    if (SLTrait.WillCharacterKnowTrait(craftingSkillType, researchLineIndex, traitIndex)) then
        return SLTrait.GetTraitKey(craftingSkillType, researchLineIndex, traitIndex), false, "AlreadyKnown"
    else
        return SLTrait.GetTraitKey(craftingSkillType, researchLineIndex, traitIndex), true
    end
end

-- returns a trait key that is unique per researchable trait
function SLTrait.GetTraitKey( craftingSkillType, researchLineIndex, traitIndex )
...
function SLTrait.GetItemCraftingSkill( itemLink )
    local itemType = GetItemLinkItemType(itemLink)
...
function SLTrait.GetResearchTraitIndex( itemLink )
...
function SLTrait.GetResearchLineIndex( itemLink )
    local craftingSkillType = SLTrait.GetItemCraftingSkill(itemLink)
    local armorType = GetItemLinkArmorType(itemLink)
...
function SLTrait.GetItemResearchInfo( itemLink )
    return SLTrait.GetItemCraftingSkill(itemLink), SLTrait.GetResearchLineIndex(itemLink), SLTrait.GetResearchTraitIndex(itemLink)
end
